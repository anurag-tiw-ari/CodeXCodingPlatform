...
const MenuBar = ({topic,title,id}) => {
  ...
  const handleContentSubmit = async () => {
    ...
    try{
    const response = await axiosClient.put(`/content/update/${id}`,{
        topic:topic,
        content:html,
        title:title?.trim()
    })
...
export default ({selectedContent}) => {
  ...
  return (
    <div className="editor-container max-w-4xl mx-auto my-8 border border-gray-200 rounded-lg shadow-sm mt-18">
      <EditorProvider 
        slotBefore={<MenuBar topic={selectedContent.topic} title={selectedContent.title} id={selectedContent._id}/>} 
